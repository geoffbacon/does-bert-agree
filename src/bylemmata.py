    ...
    for _,row in tqdm(cloze.iterrows()):
        ...
        try:  # we may have skipped this cloze example
            ...
            if np.isnan(p_correct_form):  # the correct form didn't appear in the lexicon
                ...
                if other_lemmata.empty:  # we don't have feature data on any other lemmata
                    continue
                else:
                    num_lemmata = len(other_lemmata["lemma"].unique())
                    merged = pd.merge(probs, other_lemmata, left_on=["word"], right_on=["word"])
                    ...
                    example = {"lg": lg, "uid": uid, "lemma": lemma, "correct_form": correct_form, 
                                "num_incorrect_lemmata": count, "num_lemmata": num_lemmata}
                    results.append(example)
        except FileNotFoundError:
            continue
results = pd.DataFrame(results)
results["percentage"] = 100 * (results["num_incorrect_lemmata"] / results["num_lemmata"])
